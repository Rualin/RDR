...
def parse_args():
    ...
    parser.add_argument("-llf", "--ls_label_file", type=str, help="Path to label studio label file (.json)")
    parser.add_argument("-lid", "--local_img_dir", type=str, help="Path to directory with images")
    parser.add_argument("-sd", "--save_dir", type=str, help="Path to directory where to save crops")
...
def main():
    ...
    if args.en_det:
        det_labels_file = open(os.path.join(args.save_dir, "det_labels.json"), "wt", encoding="utf-8")
